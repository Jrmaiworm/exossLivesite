...
import com.mycompany.myapp.repository.EstacaoRepository;
...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Estacao}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class EstacaoResource {
    ...
    public ResponseEntity<Estacao> updateEstacao(@PathVariable(value = "id", required = false) final Long id, @RequestBody Estacao estacao)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, estacao.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Estacao> partialUpdateEstacao(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, estacao.getId().toString())
        );
    }
