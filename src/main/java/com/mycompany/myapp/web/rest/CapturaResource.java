...
import com.mycompany.myapp.repository.CapturaRepository;
...
/**
 * REST controller for managing {@link com.mycompany.myapp.domain.Captura}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class CapturaResource {
    ...
    public ResponseEntity<Captura> updateCaptura(@PathVariable(value = "id", required = false) final Long id, @RequestBody Captura captura)
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, captura.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Captura> partialUpdateCaptura(
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, captura.getId().toString())
        );
    }
